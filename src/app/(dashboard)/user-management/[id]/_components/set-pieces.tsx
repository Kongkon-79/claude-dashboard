...
import AddEditSetPiecesForm from "./add-edit-set-pieces-form";
import { SetpiecesApiResponse, SetpiecesStats } from "@/components/types/set-pieces-data-type";

const SetPiecesPage = ({ id }: { id?: string }) => {
  console.log("view data", id)
  const [currentPage, setCurrentPage] = useState(1);
  const [deleteModalOpen, setDeleteModalOpen] = useState(false);
  const [setPiecesId, setSetPiecesId] = useState("");
  const [selectedSetPieces, setSelectedSetPieces] =
    useState<SetpiecesStats | null>(null);
  const [addSetPiecesForm, setAddSetPiecesForm] = useState(false);
  const queryClient = useQueryClient();
  console.log(queryClient)
  const session = useSession();
  const token = (session?.data?.user as { accessToken: string })?.accessToken;

  const { data, isLoading, isError, error } = useQuery<SetpiecesApiResponse>({
    queryKey: ["all-setpieces", id, currentPage],
    queryFn: async () => {
      const res = await fetch(
        `${process.env.NEXT_PUBLIC_BACKEND_URL}/setpieces/${id}?page=${currentPage}&limit=8`, {
        method: "GET",
        headers: {
              ...
              return (
                <TableRow key={index} className="">
                  ...
                  <TableCell className="text-base font-normal text-[#131313] leading-[150%] text-center py-3">
                    {item?.freekicksShots || "N/A"}
                  </TableCell>
                  <TableCell className="text-base font-normal text-[#131313] leading-[150%] text-center py-3">
                    {item?.freekicksShotsonTarget || "N/A"}
                  </TableCell>
                  <TableCell className="text-base font-normal text-[#131313] leading-[150%] text-center py-3">
                    {item?.penaltyKicks || "N/A"}
                  </TableCell>
                  <TableCell >
  ...
  const { mutate } = useMutation({
    mutationKey: ["delete-setpieces"],
    mutationFn: async (id: string) => {
      const res = await fetch(
        `${process.env.NEXT_PUBLIC_BACKEND_URL}/setpieces/${id}`,
        {
          method: "DELETE",
          headers: {
            "Content-Type": "application/json",
      ...
      <div className='pt-2'>
        ...
        {deleteModalOpen && (
          <DeleteModal
            ...
            title="Are You Sure?"
            desc="Are you sure you want to delete this Set Pieces?"
          />
        )}

        {/* add and edit fouls form  */}

        <div>
          {
            addSetPiecesForm && (
              <AddEditSetPiecesForm
                open={addSetPiecesForm}
                onOpenChange={(open: boolean) => setAddSetPiecesForm(open)}
                defaultData={selectedSetPieces}
                playerId={id}
              />
            )
          }
